...
def get_code_index_cols(dashboard_metadata):    
    code = list(dashboard_metadata['Strategy'])[0]
    indices = sorted(dashboard_metadata['Index'])
    name_columns = sorted(dashboard_metadata.keys() - {'Strategy', 'Points'})
    pnl_columns = sorted(dashboard_metadata['PL Basis'])
    return code, indices, name_columns, pnl_columns

@st.cache_data
def mapping_dashboard_files(parquet_files):
    data = []
    ...
    return pd.DataFrame(data, columns=['Index', 'Year', 'Day', 'DTE', 'PL Basis', 'FilePath'])
...
if "folder_path" in st.session_state:
    ...
    if os.path.exists(folder_path):
        ...
        if parquet_files:
            ...
            mapping_dashboard_files_df = mapping_dashboard_files(parquet_files)
            code, indices, name_columns, pnl_columns = get_code_index_cols(dashboard_metadata)
            ...
            if st.session_state['button_clicked']:
                ...
                for column in name_columns:
                    if column not in [pivot_index, pivot_column]:
                        unique_values = list(dashboard_metadata[column])
                ...
                for column in name_columns:
                    if column in top_level_filter_col:
                ...
                if filtered_parquet_files:
                    conditions = []
                    for col, vals in filtered_dict.items():
                    ...
                    filtered_data = duckdb.sql(f"SELECT * FROM read_parquet({filtered_parquet_files}) WHERE {where_sql}").pl()
                else:
